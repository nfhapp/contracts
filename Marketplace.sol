...
contract NFHMarketPlaceContract {
    struct AuctionItem {
        uint256 id;
    ...
    function buyItem(uint256 id) payable external ItemExists(id) IsForSale(id) HasTransferApproval(itemsForSale[id].tokenAddress,itemsForSale[id].tokenId){
        ...
        address payable marketplaceOwnerAddress = payable("OWNER_ADDRESS");
        uint256 payToSeller = msg.value * 98/100;
        uint256 payToOwner = msg.value * 2/100;

        itemsForSale[id].seller.transfer(payToSeller);
        marketplaceOwnerAddress.transfer(payToOwner);
